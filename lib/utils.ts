...
export const mergeArraysBlog = (
  arr1: NavLeafItem[],
  prop1: keyof NavLeafItem,
  // prop2?: keyof BlogType
) => {
  return arr1.map((item1) => ({
    ...item1,
  }));
...
export const findBlogIndex = (blogs: any[], currentPath: string) => {
  return blogs.findIndex(blog => blog.path === currentPath);
};

export const getAdjacentBlogs = (blogs: any[], currentPath: string) => {
  const currentIndex = findBlogIndex(blogs, currentPath);
  return {
